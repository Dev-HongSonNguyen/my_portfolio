...
const processEditAdmin = ({id}) => {
    ...
    useEffect(()=>{
        axios.get(`https://s2qbne-8080.preview.csb.app/api/APIprocess/${id}`).then(({data})=> setProcess(data))
    },[])
    useEffect(()=>{
        ...
        form.addEventListener("submit", function(e){
            ...
            axios.put(`https://s2qbne-8080.preview.csb.app/api/APIprocess/${id}`, form)
    ...
    <div class="max-w-6xl m-auto">
      <form action="" id="form">
          ...
          <div>
              ...
              <input id="name" "type="text" class="border w-full outline-none p-2" value="${process.name}">
          </div>
          <div>
              <label for="" class="block text-[#ffff]">Description</label>
              <textarea id="des" "type="text" class="border w-full outline-none p-2">${process.description}</textarea>
          </div>
          <div class="">
              <input type="submit"
